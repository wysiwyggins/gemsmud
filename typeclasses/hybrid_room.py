...
4) Extra commands
  ...
  CmdHybridRoomLook - look command supporting room details
  CmdHybridRoomDesc - desc command allowing to add seasonal descs,
  CmdHybridRoomDetail - command allowing to manipulate details in this room
                    as well as listing them
  CmdHybridRoomGameTime - A simple `time` command, displaying the current
                    time and season.


Installation/testing:

Adding the `HybridRoomCmdset` to the default character cmdset will add all
...
class CharacterCmdset(default_cmds.Character_CmdSet):
    ...
    def at_cmdset_creation(self):
        ...
        self.add(extended_room.HybridRoomCmdSet)  # <-new

```

Then reload to make the bew commands available. Note that they only work
on rooms with the typeclass `HybridRoom`. Create new rooms with the right
...
""" 
 ...
 """
    ...
    def return_appearance(self, looker, **kwargs):
        ...
        return super(HybridRoom, self).return_appearance(looker, **kwargs)
...
class CmdHybridRoomLook(default_cmds.CmdLook):
    """
    look
...
class CmdHybridRoomDesc(default_cmds.CmdDesc):
    """
    ...
    def func(self):
        ...
        if self.switches and self.switches[0] in ("spring", "summer", "autumn", "winter"):
            ...
            if obj.attributes.has("general_desc"):
                ...
                # this is not an HybridRoom.
                caller.msg("The description was set on %s." % obj.key)


class CmdHybridRoomDetail(default_cmds.MuxCommand):
...
class CmdHybridRoomGameTime(default_cmds.MuxCommand):
    """
    Check the game time
...
class HybridRoomCmdSet(CmdSet):
    ...
    def at_cmdset_creation(self):
        self.add(CmdHybridRoomLook)
        self.add(CmdHybridRoomDesc)
        self.add(CmdHybridRoomDetail)
        self.add(CmdHybridRoomGameTime)
